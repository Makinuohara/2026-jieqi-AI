...
- `ProtocolCodec`：负责 JSON 解析、序列化和 8 KiB 文本帧限制。
...
GUI 侧通过自绘棋盘底板叠加棋子按钮层的方式呈现棋盘，支持空位落点、吃子落点、
上一步走子路径和将军中的将/帅高亮。

当前搜索 AI 使用 Expectiminimax 概率搜索作为主干。根节点采用迭代加深：先在较浅深度
得到稳定着法，再逐层加深；每层完整搜索后会用本层分数重排下一层根节点候选，以提升
限时场景下的 Alpha-Beta 剪枝效率，并减少超时后只能退回贪心兜底的情况。

每次合法走子后，规则引擎按固定顺序处理吃将、80 半回合无吃子和棋、长将长捉以及
下一方无合法走法判负。GUI 和未来服务器只读取裁决结果，不自行重复判断终局。
将军检测用于界面提示和长将统计，根据作业规定不限制玩家选择不应将的着法。
界面每次刷新都会分别查询红帅和黑将；如果上一方没有应将，回合切换后仍会提示
哪一方被将军。

## 当前实现边界

现有版本已经实现七类棋子的基础移动、合法走法生成、暗子首次移动后随机翻开、
吃将终局、80 半回合无吃子和棋、困毙判负、长将长捉、兵卒例外、将军提示、本地
真人对搜索人工智能以及本地 AI 对 AI 基础自动对局。完整联网房间对局、棋谱持久化
和跨组联调仍未完成。
